...
import { useSubAgentStatus, type SubAgentInfo } from '../hooks/useSubAgentStatus';
...
export default function SubAgentStatusBar({ projectId, sessionId }: Props) {
  const t = useT();
  // Status fetch + WS refresh + poll live in the shared hook (the composer's
  // pinned-worker "Queue" state reads the same source — spec 074).
  const { agents, refresh } = useSubAgentStatus(projectId, sessionId);
