...
| 11 | **Connectors + the workspace layer** | 2026-07-04 · **Implementation complete** on `feature/backlog-11-12-connectors-workspace` (off backlog-9 tip; 8 commits). Phases A+B+C all landed: MCP connector core (catalog GCal/Drive available + Gmail pending-verification, PKCE OAuth BYO-client, fail-closed reflection, Tier-0 custom servers), calendar hub (EventKit + MCP sources, linkage, refresh) + full calendar surface (week grid/agenda/refresh/project linking), two-zone sidebar (Quick Tasks promoted), settings rail + connector settings, onboarding rework (sandbox step deleted → banner retry; merged Connect-Your-Accounts step); installer already ran `--setup-sandbox` elevated (verified, no change). Gates: 2172 unit + 144 platform + 759 vitest green; live-daemon smokes passed (connectors catalog, calendar routes + 422 caps, availability). Awaiting user mobile QR pass. Not yet done: Google app registration (BYO client until then), real EventKit run needs `pip install pyobjc-framework-EventKit`. Spec at [`BACKLOG/specs/011-…md`](BACKLOG/specs/011-connectors-and-workspace-layer.md). |
| 12 | **Cross-project scope for Quick Tasks** | 2026-07-04 · **Implementation complete** on `feature/backlog-11-12-connectors-workspace` (same branch as 11). Portal leak FIXED (workspace-scoped portals; read-only Seatbelt render verified), multi-root reads with `orbital/`+`.git/` exclusions and `[project: name]` labels, per-session scope (in-memory; scratch defaults ALL) + GET/PUT endpoints, scope chip UI. Live-daemon smoke: default/round-trip/409/422 all correct. Windows read-only ACL parity needs a CI run (spec §7-Q5). Awaiting user mobile QR pass. Spec at [`BACKLOG/specs/012-…md`](BACKLOG/specs/012-cross-project-scope-quick-tasks.md). |
