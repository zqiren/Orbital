...
| 30 | **BUG FIX: AGENTS.md not seeded into new project folder on creation** | 2026-07-27 · **ready-to-implement** · claude-code verdict: **never wired up — NOT a regression from item 25** (item 25 cleared). Three independent lines of evidence: (a) no Orbital code path has ever written to a project's workspace root — every writer is `orbital/`-namespaced per `ProjectPaths`; (b) the only `AGENT.md` that ever existed was a read-only prompt-bootstrap at `{workspace}/orbital/AGENT.md`, deleted in commit `ff7a4c3` (2026-04-25) as "dead weight" — and never replaced; (c) Spec 15 explicitly deferred the per-project template as a follow-on — this bug IS that follow-on resurfacing. Item 25's combined backend diff is additive-only (`agents_v2.py` +9/−4 + new `/api/v2/platform/mkdir`), zero removed lines in the create path. **Plan**: hardcoded `AGENT_MD_TEMPLATE` constant in a new `agent_os/daemon_v2/agent_md_seeder.py`, called once after `install_default_skills` in `create_project` (`agents_v2.py:541`), mirroring the swallow-and-log failure posture, with a never-clobber guard. **All decisions resolved 2026-07-27**: **D-1** ✅ filename = `AGENTS.md` (uppercase) — matches Spec 15 + community auto-read convention (note: same spelling as Spec 15's *workspace-root* `AGENTS.md` but layered — workspace root vs project root); **D-2** ✅ deletion semantics — do **not** delete `AGENTS.md` on project delete (user: 'should not delete. if it is outside of the orbital folder') — `AGENTS.md` at project root is outside `orbital/` and is therefore user-owned content; aligns with the existing 'never touch user files outside `orbital/`' contract that `_cleanup_project_files` already enforces; **D-3** ✅ Quick Tasks / scratch — **skip** seeding (user: 'quick task shouldnt need one') — a fresh empty workspace has nothing meaningful for `AGENTS.md` to describe; matches the existing pattern where `install_default_skills` also skips scratch (`default_skills_installer.py:153-154`). XS–S — XS code step, but ~60 lines of template content must be authored from scratch. Full spec at [`BACKLOG/specs/030-agents-md-seeding-on-project-creation.md`](BACKLOG/specs/030-agents-md-seeding-on-project-creation.md). **2026-08-02 · IMPLEMENTED on `feature/backlog-30-26-27-28`** (`a613ea1`): `agent_md_seeder.py` + `create_project` wiring + 8 tests + README layout tree; template deliberately omits absolute paths/timestamps (committable in the user's repo, spec R-4 wins over §2's workspace-path line). Live-daemon smoke verified (fresh temp-workspace project → AGENTS.md seeded with substituted names). Awaiting release verification. |
| 28 | **Sub-agent lifecycle wake coverage: close the remaining deferred-terminal gaps** | 2026-07-22 · Surfaced by item 23's implementation + whole-branch review. Item 23's D1 wakes the manager only for error/failed/stopped. Three confirmed siblings still hit the deferred-never-wakes gap: (a) `on_completed` (mid-turn completion appended silently), (b) `on_turn_interrupted` (its own docstring calls silence there "the Piece-3 Part-C silent-hang class"), (c) the fanout join summary (`fanout.py:399` injects with NO wake tag — a fanout whose last worker terminates mid-turn, incl. all-failed, never wakes the owner). Mechanism already exists (the `sub_agent_terminal` meta tag + teardown wake from #23) — this is tagging the remaining producers + tests. Also add to the release checklist: one live mid-turn sub-agent-error smoke (the #23 wake is unit-proven via the real defer path but has never been exercised under a live LLM dispatch). XS–S. **2026-08-02 · IMPLEMENTED on `feature/backlog-30-26-27-28`** (`2e83e29`): all three producers tagged (kinds `completed`/`interrupted`/`fanout_join`, display_content splits preserved); consumer widened from the error/failed/stopped kind list to event-only match (an enumeration is what let tagged producers stay ignored); #23's completed-does-not-wake scope guard rewritten to untagged-does-not-wake; stale `tests/regression` defer fixture fixed (predates `e885184`'s meta= threading); live mid-turn terminal smoke added to CLAUDE.md release pre-flight (step 4). Behavior note: ordinary mid-turn completions now hot-resume the manager. Awaiting release verification incl. that live smoke. |
| 27 | **Three [Sub-agent] marker shapes render as nothing + parity fixture extension** | 2026-07-22 · Whole-branch review of #23 found `on_user_stopped` ("stopped by user."), `on_turn_interrupted`, and `on_background_work_lost` write [Sub-agent] markers with no renderer rule and no parity-fixture row — they render as NOTHING on history reload (pre-existing; exactly the #23-D2 bug class). Add renderer rules + fixture rows. Known limitation to address while there: the parity pytest's producer set is hand-maintained (a brand-new producer shape isn't auto-caught — only drift in the 6 known shapes is). XS. **2026-08-02 · IMPLEMENTED on `feature/backlog-30-26-27-28`** (`7e8f617`), scope grew twice with coordinator approval: (1) a FOURTH silent shape, `on_interaction_required` — the manager blocked on user input rendering as timeline silence — was found by the new guard and covered; (2) the fix needed the granted ChatView.tsx label-mapping region, whose unguarded `/* failed */` fallback would have rendered every new action as a red "failed" lie — now an exhaustive switch with a compile-time `never` proof, honest generic fallback, and severity-ranked tones (user's own stop = neutral; destroyed background work / interruption = amber; blocked-on-input = accent). Hand-maintained producer list is gone: an AST scan of lifecycle_observer.py fails pytest for any `[Sub-agent]` producer lacking a fixture row (plain, f-string, or concatenated). Fixtures 6→11; render-level tests drive the same fixture file through the real component. Follow-up → item 35. |
| 26 | **Create-modal + folder-picker polish batch** | 2026-07-22 · Follow-ups from item 25's reviews, none blocking: (a) `FolderPickerModal` now has zero production call sites — delete the wrapper + its test + orphaned i18n keys (`folderPicker.{title,shortcuts,recent,select}`) or record a deliberate keep; (b) mkdir endpoint: require `parent` to be absolute (400) for self-consistency with the picker's always-absolute paths; (c) modal a11y batch: `role=dialog`, Escape-to-close, focus trap, autoFocus — autoFocus MUST use WKWebView-safe focus (`preventScroll` + double-rAF; modal uses `animate-slide-up`, see the WKWebView compositor notes); (d) queued-then-dropped @mention leaves zero dispatch markers in session JSONL (user msg itself is persisted; only the marker record is lost). XS–S. **2026-08-02 · IMPLEMENTED on `feature/backlog-30-26-27-28`** (`4b27b2e`): (a) FolderPickerModal + test deleted; 4 orphaned keys removed from strings.ts AND the source CSV (gen-i18n full-rewrites strings.ts, so stale CSV rows would resurrect them); the 11 keys FolderBrowserPanel consumes kept. (b) mkdir 400s on non-absolute `parent` (before the is_dir check; Windows-safe tests). (c) role=dialog/aria-modal/focus trap/Escape/focus-restore on the create modal; autofocus is WKWebView-safe (double-rAF + preventScroll — modal mounts translated off-screen mid slide-up); Escape scoped OUT of the embedded picker so one keypress can't cancel the inline folder editor AND destroy the half-filled form. (d) premise correction: the mention queue is sub_agent_manager.py's `_prompt_queues`, not agent_manager's `_pending_inject`; **all FIVE** silent drop sites (turn-closed transport-death, turn-closed broken-adapter, stop()/stop_all(), and the background-send exception path found in coordinator review) now mark each dropped prompt via the existing `on_failed` shape with honest attribution, one row per prompt (each has its own orphaned user bubble); stop() marks before the kill so rows survive an overrun teardown; durable-row-in-JSONL regression test included. Reuse-of-red-failed-shape nit + i18n orphans → item 35. |
