...
| 40 | **Cost-loading flash in BudgetCorner + QueueTab (follow-up from #36)** | 2026-08-03 · #36 fixed the settings page's false "$0.00" / "no sub-agents" states only (its decision 3); the other `useCost` consumers still paint a fabricated zero during the first fetch. Same pending/failed gate as `BudgetSection`; consider fixing `useCost`'s `loading: false` initial state at the source now that all consumers would move together. XS. |
| 39 | **Cache-aware pricing: add `cached_input_per_1m` for DeepSeek/MiniMax so the budget ledger stops billing cache hits at the flat input rate** | 2026-08-03 · Follow-up from #38 §5 fix 3 (decision Q3: separate item). `providers.json` carries no cached rate for DeepSeek/MiniMax, so `budget_spent_usd` is cache-blind — and once #38 ships and the hit rate jumps ~23%→~90%, the overbilling becomes material (hits bill ~10× cheaper than the flat rate). S. |
