...
# ===========================================================================
# AC-1: build() returns tuple of 3 non-empty strings
# ===========================================================================

class TestBuildReturnType:

    def test_build_returns_tuple_of_three_strings(self):
        builder = PromptBuilder()
        ctx = make_context()
        result = builder.build(ctx)
        assert isinstance(result, tuple)
        assert len(result) == 3
        cached, semi_stable, dynamic = result
    ...
    def test_build_neither_is_empty(self):
        ...
        assert len(semi_stable.strip()) > 0, "semi_stable should not be empty"
...
class TestSubAgentsSection:
    ...
    def test_no_sub_agents_section_when_empty(self):
        ...
        assert "sub-agent" not in semi_stable.lower() or "Sub-agents available" not in semi_stable
    ...
    def test_sub_agents_section_present_when_non_empty(self):
        ...
        _, semi_stable, _ = builder.build(ctx)
        assert "claudecode" in semi_stable
...
class TestCacheStability:
    ...
    def test_identical_context_identical_cached_prefix(self):
        builder = PromptBuilder()
        ctx = make_context()
        cached1, _, _ = builder.build(ctx)
        cached2, _, _ = builder.build(ctx)
...
class TestCacheSplitBehavior:
    ...
    def test_different_datetime_cached_same_dynamic_different(self):
        ...
        cached1, _, dynamic1 = builder.build(ctx1)
        cached2, _, dynamic2 = builder.build(ctx2)
...
class TestPromptBuilderWithWorkspace:
    ...
    def test_builder_memory_section_in_semi_stable(self):
        builder = PromptBuilder()
        ctx = make_context()
        _, semi_stable, _ = builder.build(ctx)
        lower = semi_stable.lower()
...
class TestSkillsDisciplineInjection:
    ...
    def test_no_skill_loader_returns_no_skills_or_discipline(self):
        ...
        ctx = make_context()
        _, semi_stable, _ = builder.build(ctx)
        assert "## Skills" not in semi_stable
