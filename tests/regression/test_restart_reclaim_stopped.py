...
"""Phase 5: at daemon startup, a queue in PAUSED state must NOT be
...
def test_paused_queue_preserves_running_item(tmp_path):
    ...
    dispatcher = QueueDispatcher(
        project_id="proj_paused_reclaim",
        store=store,
        agent_manager=mgr,
    )

    summary = dispatcher.reclaim_on_startup()
    assert summary["queue_state"] == "paused"
    assert summary["reclaimed_items"] == []
    assert summary["blocked_items"] == []

    state = store.load()
    # Item is STILL running; queue is STILL paused; attempt NOT closed
