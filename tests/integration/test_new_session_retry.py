...
def _write_session_file(workspace: str, session_id: str,
    ...
    filepath = ProjectPaths(workspace).session_file(session_id)
...
def test_new_session_llm_retry_on_timeout_via_http(caplog):
    ...
    with tempfile.TemporaryDirectory() as workspace:
        ...
        _write_session_file(workspace, "old_session_http_retry", old_messages)

        from agent_os.daemon_v2.agent_manager import ProjectHandle
        from agent_os.agent.session import Session

        session = Session.new("old_session_http_retry", workspace)
